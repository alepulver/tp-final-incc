...
class CollectionVocabularyAnalyzer:
    ...
    # TODO: optimize these, avoid countin for each word
...
class DataTable:
    def __init__(self, extractor, collection):
        ...
        self._features = {}
        for book in self._collection.books():
            self._features[book.title()] = self._extractor.extract_from(book)

        self._authors = {}
        for a in self._collection.authors():
            self._authors[a] = [b.title() for b in self._collection.books_by(a)]

    def as_dataframe(self):
        frames = []
        for book,features in self._features.items():
            df = pandas.DataFrame(list(features.values()),
                index=list(features.keys()), columns=[book])
            frames.append(df)
        return pandas.concat(frames)
